#!/usr/bin/env python
# Performance test various xml parsers
import zlib
import sys
import traceback
import re
...
def parse_listing_xml (context, fragment):
    ...
    try:
        ...
        #namespaces = {'commons': 'http://rets.org/xsd/Syndication/2012-03'}
        ...
        #ET.register_namespace('commons', 'http://rets.org/xsd/RETSCommons')

        # Parse the xml fragment
        root = ET.fromstring(xml)

        #el_brokerage = get_nselement(root[0], ns, 'Brokerage')

        listing = {}
        #listing = isNowListed.listing.Listing()
        # listing = Listing()
        #
        # el_listing = get_ns_element(dom, 'Listing')
        el_listing = root[0]
        if el_listing:
            ###################
            # Brokerage block #
